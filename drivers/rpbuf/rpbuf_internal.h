...
#define RPBUF_MEASURE_TIME_RESERVED			(sizeof(uint64_t) * 16)
...
extern void *g_rpbuf_amp_ts_dev;
...
#else
static inline uint64_t trace_rpbuf_get_time(void)
{
	return 0;
}
static inline void put_trace_rpbuf_time(struct rpbuf_buffer *buffer, int idx, uint64_t time) { }
static inline void trace_rpbuf_time(struct rpbuf_buffer *buffer, int idx) { }
#endif /* IS_ENABLED(CONFIG_AW_RPBUF_PERF_TRACE) */
