...
#define SUNXI_RPBUF_CORE_VERSION "1.2.7"
		...
		struct list_head *dummy_buffers, const char *name)
{
	...
	list_for_each_entry(buffer, dummy_buffers, dummy_list) {
		if (strncmp(buffer->name, name, RPBUF_NAME_SIZE) == 0)
			return buffer;
	}
	return NULL;
}
							...
							const char *name, int len)
{
	...
	if (name_len == 0 || name_len >= RPBUF_NAME_SIZE) {
		dev_err(dev, "invalid buffer name \"%s\", its length should be in range [1, %d)\n",
			name, RPBUF_NAME_SIZE);
		goto err_free_rpbuf_buffer;
	}
	strscpy(buffer->name, name, RPBUF_NAME_SIZE);
...
#if IS_ENABLED(CONFIG_AW_RPBUF_PERF_TRACE)
...
static struct rpbuf_link *rpbuf_create_link(void *token)
{
	...
	link = kzalloc_obj(struct rpbuf_link, GFP_KERNEL);
...
struct device *
rpbuf_bus_find_device_by_of_node(const struct bus_type *bus,
...
int rpbuf_register_service(struct rpbuf_service *service, void *token)
{
	...
	pr_devel("register service 0x%p (token: 0x%p)\n", service, token);
	...
	if (!link) {
		...
		pr_devel("create new link 0x%p (token: 0x%p)\n", link, token);
	...
	pr_devel("link: service: 0x%p, controller: 0x%p, ref_cnt: %d\n",
...
void rpbuf_unregister_service(struct rpbuf_service *service)
{
	...
	pr_devel("unregister service 0x%p\n", service);
			      ...
			      void *token, enum rpbuf_role role)
{
	...
	pr_devel("register controller 0x%p (token: 0x%p)\n", controller, token);
...
void rpbuf_unregister_controller(struct rpbuf_controller *controller)
{
	...
	pr_devel("unregister controller 0x%p\n", controller);
				...
				enum rpbuf_service_command cmd, void *content)
{
	...
	if (!service) {
		pr_err("link 0x%p has no service\n", link);
		spin_unlock_irqrestore(&link->lock, flags);
		ret = -ENOENT;
		goto out;
	}
							...
							void *content)
{
	...
	if (!controller) {
		dev_err(service->dev, "service 0x%p not linked with controller\n",
			service);
		spin_unlock_irqrestore(&link->lock, flags);
		ret = -ENOENT;
		goto unlock_controller_lock;
	 ...
	 */
	idr_for_each_entry(&controller->buffers, buffer, id) {
		if (strncmp(buffer->name, cont->name, RPBUF_NAME_SIZE) == 0) {
			dev_info(controller->dev,
				 "(unexpected) remote re-allocating buffer \"%s\" (available: %d)\n",
			...
			strscpy(content_back.name, buffer->name, RPBUF_NAME_SIZE);
			...
			ret = rpbuf_notify_by_link(link,
						   ...
						   (void *)&content_back);
			if (ret < 0) {
				dev_err(controller->dev, "rpbuf_notify_by_link BUFFER_CREATED failed: %d\n",
					ret);
				goto out;
			}
			ret = 0;
	...
	if (buffer) {
		...
		if (buffer->len != cont->len) {
			dev_err(controller->dev, "buffer length not match with remote (local: %d, remote: %u)\n",
				buffer->len, cont->len);
			ret = -EINVAL;
			goto unlock_controller_lock;
		}
		...
		buffer = rpbuf_find_dummy_buffer_by_name(&controller->remote_dummy_buffers,
							 cont->name);
		if (buffer) {
			dev_info(controller->dev, "buffer \"%s\": (unexpected) overwrite information in 'remote_dummy_buffers'\n",
				buffer->name);
			strscpy(buffer->name, cont->name, RPBUF_NAME_SIZE);
			buffer->len = cont->len;
			buffer->id = cont->id;
			buffer->pa = cont->pa;
			buffer->da = cont->da;
	...
	if (is_available && buffer->offline) {
		...
		ret = rpbuf_notify_by_link(link, RPBUF_SERVICE_CMD_BUFFER_CREATED, (void *)&content_back);
		if (ret < 0) {
			dev_err(controller->dev, "rpbuf_notify_by_link BUFFER_CREATED failed: %d\n", ret);
			goto out;
		}
							  ...
							  void *content)
{
	...
	if (buffer) {
		...
		if (buffer) {
			...
			dev_err(controller->dev, "buffer not found, nothing handled with BUFFER_DESTROYED\n");
			ret = -EINVAL;
			goto err_out;
		}
	}
							    ...
							    void *content)
{
	...
	u32 timestamp = trace_rpbuf_get_time();
...
int rpbuf_service_get_notification(struct rpbuf_service *service, void *msg, int msg_len)
{
	...
	if (header->content_len != rpbuf_service_message_content_len[cmd]) {
		dev_err(dev, "rpbuf service message content length not match (command: %d, request: %d, actual: %d)\n",
...
static int rpbuf_free_buffer_internal(struct rpbuf_buffer *buffer, int do_notify)
{
	...
	if (dummy_buffer) {
		...
		if (dummy_buffer) {
			 ...
			 * information.
			 */
			dev_info(dev, "buffer \"%s\": (unexpected) overwrite information in 'remote_dummy_buffers'\n",
				buffer->name);
			strscpy(dummy_buffer->name, buffer->name, RPBUF_NAME_SIZE);
					...
					void *priv)
{
	...
	idr_for_each_entry(&controller->buffers, dummy_buffer, id) {
		if (strncmp(dummy_buffer->name, name, RPBUF_NAME_SIZE) == 0) {
	...
	if (dummy_buffer) {
		...
		if (buffer->len != dummy_buffer->len) {
			dev_err(dev, "buffer length not match with remote (local: %d, remote: %d)\n",
	...
	strscpy(content.name, buffer->name, RPBUF_NAME_SIZE);
	content.id = buffer->id;
...
#ifdef CONFIG_AW_RPBUF_PERF_DATA_AUTO_DUMP
	rpbuf_perf_data_t perf_data;
#endif
...
#ifdef CONFIG_AW_RPBUF_PERF_DATA_AUTO_DUMP
	ret = rpbuf_get_perf_data(buffer, &perf_data);
	if (ret) {
		dev_err(dev, "get rpbuf performance data failed, ret: %d\n", ret);
	} else {
		dev_info(dev, "'%s' performance data:\n", buffer->name);
		rpbuf_dump_perf_data(&perf_data);
	}
#endif
