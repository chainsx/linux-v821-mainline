...
int xradio_tx_cmd_ack(struct xradio_hw *xradio_hw, void *data, u16 data_len)
{
	...
	if (ret)
		...
		txrx_printk(XRADIO_DBG_MSG, "cmd ack send success.\n");
...
// return > 0: need send to next process; < 0: pre process err; = 0: pre process success
...
static inline bool xradio_check_tx_retry(int status)
{
