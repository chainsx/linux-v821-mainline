...
int xradio_platform_init(struct xradio_plat **plat)
{
	...
	xradio_plat = kzalloc_obj(struct xradio_plat, GFP_KERNEL);
	if (!xradio_plat) {
	...
	bus_if = kzalloc_obj(struct xradio_bus, GFP_KERNEL);
	if (!bus_if) {
	...
	*plat = xradio_plat;
	...
	plat_printk(XRADIO_DBG_ALWY, "xradio_platform_init success 0x%x\n", (u32)xradio_plat);
	return 0;
