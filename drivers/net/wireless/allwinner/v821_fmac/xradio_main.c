...
#define V821_SID_COMPATIBLE	"allwinner,sunxi-sid"
...
module_param_array_named(macaddr, xradio_macaddr_param, byte, NULL, 0444);
#else
/* insmod v821_fmac.ko macaddr=xx:xx:xx:xx:xx:xx */
static char *xradio_macaddr_param;
module_param_named(macaddr, xradio_macaddr_param, charp, 0444);
#endif
...
static int xradio_iface_set_mac_address(struct net_device *ndev, void *data)
{
	...
	dev_addr_set(ndev, mac_addr->sa_data);
	return 0;
}
...
int xradio_iface_net_set_mac(void *priv, u8 *mac)
{
	...
	dev_addr_set(net_priv->ndev, mac);
	return 0;
}
...
void *xradio_iface_add_net_dev(void *priv, u8 if_type, u8 if_id, char *name, xr_net_data_tx_t tx_func)
{
	...
	dev_addr_set(ndev, net_priv->mac_address);
...
static int xradio_get_efuse(struct xradio_plat *priv)
{
	...
	dev_node = of_find_compatible_node(NULL, NULL, V821_SID_COMPATIBLE);
