...
static inline int rpbuf_send(struct device *dev, struct xradio_rpbuf_dev *rpbuf_dev)
{
	int ret, i;
	struct rpbuf_buffer *buffer = rpbuf_dev->buffer_mtx;
	struct xradio_agg_hdr *agg_hdr;
...
static int xradio_rpbuf_probe(struct platform_device *pdev)
{
	...
	strscpy(rpbuf_dev->name_mrx, RPBUF_BUFFER_NAME_XRADIO_RX,
		RPBUF_NAME_SIZE);
	rpbuf_dev->len_mrx = XRADIO_MAX_RX_RPBUF_SIZE;
	strscpy(rpbuf_dev->name_mtx, RPBUF_BUFFER_NAME_XRADIO_TX,
		RPBUF_NAME_SIZE);
	...
	rpbuf_printk(XRADIO_DBG_MSG, "xrlink %s rpbuf success!\n", RPBUF_BUFFER_NAME_XRADIO_RX);
...
static void xradio_rpbuf_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct xradio_rpbuf_dev *rpbuf_dev = dev_get_drvdata(dev);
