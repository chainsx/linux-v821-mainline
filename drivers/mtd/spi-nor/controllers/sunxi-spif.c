...
#define SUNXI_SPIF_MODULE_VERSION	"1.0.3"

static bool double_clk_flag;

static int sunxi_spif_debug_mask;
module_param_named(spif_debug_mask, sunxi_spif_debug_mask, int, 0664);

static void __maybe_unused sunxi_spif_dump_reg(struct sunxi_spif *sspi)
{
	char buf[1024] = {0};
...
static void __maybe_unused sunxi_spif_dump_descriptor(struct sunxi_spif *sspi)
{
	char buf[512] = {0};
	int desc_num = 0;
	...
	while (desc_num < DESC_PER_DISTRIBUTION_MAX_NUM) {
		snprintf(buf, sizeof(buf)-1,
			...
			printk("sspi->dma_desc addr [%llx]...\n\n",
					(unsigned long long)sspi->desc_phys[desc_num]);
...
static u32 sunxi_spif_get_version(struct sunxi_spif *sspi)
{
	return readl(sspi->base_addr + SPIF_VER_REG);
}
...
static int sunxi_spif_clk_init(struct sunxi_spif *sspi)
{
	...
	if (ret) {
		sunxi_err(&sspi->pdev->dev, "failed to do reset %d\n", ret);
		return -1;
	}
...
static void __maybe_unused sunxi_spif_config_tc(struct sunxi_spif *sspi)
{
...
static int sunxi_spif_handler_descline(struct sunxi_spif *sspi, struct spi_mem_op *op)
{
	...
	while (total_data_len) {
		...
		dma_desc->addr_dummy_data_count &= ~DMA_TRANS_NUM;
...
static int sunxi_spif_mem_exec_op(struct sunxi_spif *sspi, struct spi_mem_op *op)
{
	...
	if (op->addr.nbytes) {
		...
		if (op->addr.nbytes == 4) { //set 4byte addr mode
			if (sunxi_spif_get_version(sspi) >= CONFIG_SUNXI_SPIF_V2) {
				sspi->dma_desc[0]->addr_dummy_data_count &= ~SPIF_ADDR_SIZE_MASK_V2;
				sspi->dma_desc[0]->addr_dummy_data_count |= SPIF_ADDR_SIZE_32BIT_V2;
			} else {
				sspi->dma_desc[0]->addr_dummy_data_count &= ~SPIF_ADDR_SIZE_MASK;
				sspi->dma_desc[0]->addr_dummy_data_count |= SPIF_ADDR_SIZE_32BIT;
			}
		} else {
			if (sunxi_spif_get_version(sspi) >= CONFIG_SUNXI_SPIF_V2) {
				sspi->dma_desc[0]->addr_dummy_data_count &= ~SPIF_ADDR_SIZE_MASK_V2;
				sspi->dma_desc[0]->addr_dummy_data_count |= SPIF_ADDR_SIZE_24BIT_V2;
			} else {
				sspi->dma_desc[0]->addr_dummy_data_count &= ~SPIF_ADDR_SIZE_MASK;
				sspi->dma_desc[0]->addr_dummy_data_count |= SPIF_ADDR_SIZE_24BIT;
			}
		}
		/* set addr buswidth */
		sspi->dma_desc[0]->cmd_mode_buswidth |=
...
static int sunxi_spif_resource_get(struct sunxi_spif *sspi)
{
	...
	if (IS_ERR(sspi->rstc)) {
		...
		sunxi_err(&sspi->pdev->dev, "failed to get reset control %d\n", ret);
		return -EINVAL;
	}
...
static int sunxi_spif_nor_register(struct sunxi_spif *sspi)
{
	...
	int ret;
		const char * const parsers[] = { "ofpart", NULL };
	...
	return mtd_device_parse_register(&nor->mtd, parsers, NULL, NULL, 0);
}
...
static const struct of_device_id sunxi_spif_dt_ids[] = {
	...
	{.compatible = "allwinner,sun300i-spif", .data = &sun55iw3_spif_data},
	{ /* sentinel */ },
};
...
static int sunxi_spif_probe(struct platform_device *pdev)
{
	...
	sspi = devm_kzalloc(&pdev->dev, sizeof(struct sunxi_spif), GFP_KERNEL);
	if (!sspi) {
	...
	for (i = 0; i < DESC_PER_DISTRIBUTION_MAX_NUM; i++) {
		...
		sunxi_debug(&pdev->dev, "dma descriptor phys addr is %llx\n",
				(unsigned long long)sspi->desc_phys[i]);
	}
...
static void sunxi_spif_remove(struct platform_device *pdev)
{
	...
	mtd_device_unregister(&sspi->nor->mtd);
	sunxi_spif_hw_deinit(sspi);
	sunxi_spif_resource_put(sspi);
	for (i = 0; i < DESC_PER_DISTRIBUTION_MAX_NUM; i++)
...
static void sunxi_spif_restore_register(struct sunxi_spif *sspi)
{
	int i;
	for (i = 0; i < 24; i++)
		writel(sspi->spif_register[i], sspi->base_addr + 0x04 * i);
}
